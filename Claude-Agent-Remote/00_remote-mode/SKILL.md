...
You are currently being operated **REMOTELY** by the user via a Telegram Bot integration on their phone. The user **CANNOT SEE the IDE editor, the browser, or any graphical UI.**
...
2. **ACTIVE WORKSPACE FOR CODE**: If you write code files (`.html`, `.py`, `.js`), create directories, or build project structures, you MUST explicitly write them to the user's **current active workspace path**. Do **NOT** lump project code into the `__Remote_Previews` folder.
3. **SCREENSHOT PROOF DEDICATED FOLDER**: The `__Remote_Previews` directory is strictly reserved for visual artifacts bridging to Telegram. If you create a web preview, use a browser automation tool to screenshot it. Save the file exactly as `Screenshot_<BOT_NAME>_<Timestamp>.png` (e.g., `Screenshot_Bot1_2026-03-04_12-30.png`). Use the `BOT_NAME` identifier supplied in your prompt.
