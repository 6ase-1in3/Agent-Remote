...
1. **📱 Telegram 全域遠端遙控**
   ...
   - **[接收成果與狀態]**：對話與執行摘要傳至 Telegram（附上精準的工作區名稱與已執行時間），圖片成品則自動存入 OneDrive 雲端預覽。
   - **[發送素材]**：完全支援在 Telegram 上傳送圖片給 AI，系統會自動載回電腦裡的本機工作區進行視覺解析。
   - **[智慧防卡死機制]**：當碰上 API 斷線等錯誤而跳出 Retry 按鈕時，腳本會「自動幫您重試」並回報手機，無須親自干預；當重試過於頻繁或出現需要切換模型的 Upgrade 提示時，亦會傳送警告至手機，提醒您接手處理。
   - **[遠端緊急應變指令]**：在 Telegram 中輸入 `/retry` 可直接從遠端強制點擊重試按鈕；輸入 `/model` 可自動抓取並點擊切換模型選單，解決各種中斷盲區。
...
### 3. 動態資源管理 (NEW!) 🚀
除了 `.bat` 一鍵生成外，現在支援更靈活的外部配置：

- **`click_keywords.txt`**: 自動點擊的關鍵字清單。每行一個關鍵字（如 `Accept`, `Yes`, `確認`）。修改此檔案後執行 `.bat`，所有腳本會自動同步。
- **自定義技能索引**: 自動從指定 Markdown 檔案解析所有 `00_xxxxx` 格式的技能，無需手動修改 Python。
